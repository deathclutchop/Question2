...
struct info readyqueue[100];//ready queue,front,rear.
int rqr=-1,rqf=-1;
//renq() will add the process to readyqueue
...
//rdq() will remove process from readyqueue
void rdq()
{
if(rqr==rqf)
	{
...
//updateq() will check for the processes arrived and adds them to readyqueue 
...
int lrtf()
{
	...
	for(i=rqf;i<=rqr;i++)
	{
		if(readyqueue[i].bursttime>readyqueue[index].bursttime&&readyqueue[i].bursttime!=0)
		{
			index=i;
		}
	}
...
void gantt(struct info p[])
{
    ...
    for(j=0;j<24;j++)
    {
        chart[j]=readyqueue[rqf].processid;
        readyqueue[rqf].bursttime--;
        qt1++;
        updateq(p,j+1);
        if(readyqueue[rqf].bursttime==0)
        {
            rdq();
            qt1=0;
        }
    ...
    {
        int lindex=lrtf();
        chart[j]=readyqueue[lindex].processid;
        readyqueue[lindex].bursttime--;
    }
